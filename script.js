...
next.addEventListener('click', () => {
   body.style = "background-color: " + '#' + (Math.random().toString(16) + '000000').substring(2, 8).toUpperCase();
   if (isLanguage) {
      getDataRU();
   } else {
      getData();
...
function getDataRU() {
   const indx = Math.floor(Math.random() * quoteRu.length);
   text.textContent = quoteRu[indx].text;
   author.textContent = "@" + quoteRu[indx].author;
}

buttonLanguage.addEventListener('click', (event) => {
   const languageButton = event.target;
   let lang = languageButton.dataset.language;
   if (!languageButton.classList.contains('lg-set')) {
      language.forEach((button, i) => language[i].classList.remove('lg-set'));
      languageButton.classList.add('lg-set');
   }
   if (lang === 'ru') {
      getDataRU();
      isLanguage = true;
